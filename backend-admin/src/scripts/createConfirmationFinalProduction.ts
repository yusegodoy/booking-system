...
async function createConfirmationFinalProduction() {
  try {
    ...
    if (existingTemplate) {
      console.log('⚠️ Template "Confirmation Final" already exists in production');
      ...
      console.log('✅ Template "Confirmation Final" updated in production');
    } else {
      // Create new template
      ...
      await template.save();
      console.log('✅ Template "Confirmation Final" created in production database');
    }
    ...
    console.log('   - HTML Content: ' + getModernHTMLTemplate().length + ' characters');
    console.log('   - Text Content: ' + getModernTextTemplate().length + ' characters');

    // Verify the template was created
    const verifyTemplate = await EmailTemplate.findOne({ name: 'Confirmation Final' });
    if (verifyTemplate) {
      console.log('\n✅ Verification successful: Template found in production database');
      console.log(`   Created: ${verifyTemplate.createdAt}`);
    ...
    console.error('❌ Error creating template in production:', error);
  } finally {
...
// Run the script
createConfirmationFinalProduction();
